...
describe('TLS version mirroring', () => {
    ...
    describe('reads the version band from every source the spec offers', () => {
        ...
        it('refuses when every version listed is one we cannot offer', () => {
            const error = expectCannotImpersonate({
                ...TLS13_SPEC,
                supportedVersions: [0x0a0a, 0x0300], // GREASE + SSLv3
            });

            expect(error.message).to.contain('no TLS version can be offered');
        });
    });
    ...
    describe('limits its version floor according to the security mode, not just the hello', () => {
        ...
        it('refuses to mirror a TLS 1.0-only hello in secure mode', () => {
            // Raising the offer to TLS 1.2 would mean mirroring this client's ciphers and
            // extensions onto a version it never asked for: worthless as a fingerprint,
            // and a combination that was never designed to go together.
            const error = expectCannotImpersonate({ ...TLS12_ONLY_SPEC, legacyVersion: TLS1_0 });

            expect(error.code).to.equal('ERR_CANNOT_IMPERSONATE');
            expect(error.message).to.contain("'secure' mode");
        });

        it('refuses to mirror a TLS 1.1-only hello in secure mode', () => {
            expectCannotImpersonate({ ...TLS12_ONLY_SPEC, legacyVersion: TLS1_1 });
        });

        it('refuses to mirror an SSLv3 hello even in insecure mode', () => {
            const error = expectCannotImpersonate(
                { ...TLS12_ONLY_SPEC, legacyVersion: 0x0300 }, { security: 'insecure' }
            );

            expect(error.message).to.contain("'insecure' mode");
        });

        it('still mirrors a TLS 1.2 hello in secure mode (the band overlaps)', () => {
            const { tlsOptions } = impersonate(TLS12_ONLY_SPEC);

            expect(tlsOptions.secureContext).to.not.equal(undefined);
        });
    ...
    describe('takes its version floor from the cipher suites too', () => {
        ...
        it('refuses a hello that offers no version its cipher suites can negotiate', () => {
            // TLS 1.3 suites but no supported_versions: it claims TLS 1.2 at best, yet
            // carries nothing that could be negotiated there.
            const error = expectCannotImpersonate(tls13CiphersOnly());

            expect(error.message).to.contain('no TLS version can be offered');
        });

        it('refuses a hello with no reproducible cipher suites at all', () => {
            const error = expectCannotImpersonate({
                ...TLS12_ONLY_SPEC,
                cipherSuites: [0x0099, 0x0098], // Not real cipher suites
            });
    ...
    describe('declines rather than throwing when nothing is left to mirror', () => {

        it('refuses a hello whose cipher suites are all gone from this OpenSSL', () => {
